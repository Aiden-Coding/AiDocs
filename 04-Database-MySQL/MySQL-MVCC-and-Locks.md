...
eadView 是事务在进行快照读（SELECT）时产生的一个结构，用于判断当前事务能看到版本链中的哪个版本。
ReadView 包含 4 个核心字段
...
如果 `trx_id < min_trx_id`：说明这个版本在创建 ReadView 之前就已经提交了，**可见**

如果 `trx_id >= max_trx_id`：说明这个版本是在创建 ReadView 之后才开启的事务修改的，**不可见**
