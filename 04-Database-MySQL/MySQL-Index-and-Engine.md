...
**B+树（B+Tree）**：
   ...
   1. **叶子节点之间通过双向链表相连**，这使得范围查询和排序变得极其高效，只需在叶子节点层进行链表遍历即可，无需返回上层节点。
...
**InnoDB 的聚簇索引选择规则**：
 ...
 1. 如果连唯一索引也没有，InnoDB 会在后台自动生成一个 6 字节的隐式自增 row_id 作为聚簇索引。
...
ELECT name, age FROM user WHERE name = 'Jack'; -

ELECT name, age, address FROM user WHERE name = 'Jack'; -
