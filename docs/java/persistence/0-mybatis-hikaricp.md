...
HikariCP 能够击败传统的 C3P0、DBCP 和 Druid 本质并非依靠硬件优势，而是将 Java 并发包（JUC）极致应用 to 其数据结构和线程模型中的神操作：
...
- **第三层：`HandOff` 等待同步机制**：如果仍然连不上说明整个连接池被全部抢占尽，利用高能无锁让步通道 `SynchronousQueue` 触发毫秒级等待超时移交机制。

---

## 二、 MyBatis 源码架构与插件深度开发

MyBatis 底层极其经典地使用了 JDK 的动态反射代理机制。我们写的一个没有实现类的 `UserMapper` 接口，在运行时却能无缝调用并返回我们想要的数据，全靠一套优雅的拦截过滤链路：

### 1. 代理织入全景图

当我们在 Spring 容器中调用 `userMapper.selectById(1)` 这一业务逻辑时，其核心工作流经历了以下 5 个阶段：
...
- **灾难重演**：由于 MySQL 底层将数值传递给 Varchar 时抛出隐式类型匹配转换规则，其在底层会将每一行的 `id_card` 隐式使用 `CAST(id_card AS SIGNED)` 函数进行运行时重构匹配。该隐式计算对该列的所有索引结构造成毁灭性打击，导致索引失效而直接引发全表扫描，线上连接数和 CPU 瞬时飙至 100%。
...
- **严重危害**：大流量高并发下，MyBatis 底层 `ResultSetHandler` 从物理连接池套接字拉取所有海量记录行后，会在堆内存中由于反射而创造数百万个高占用体积的 POJO 记录对象。由于新生代、老年代空间被瞬间塞爆，且全部是强引用无法回收，极易疯狂引发 GC (Fine GC/Full GC) 导致整机挂起，紧接着拋出 `java.lang.OutOfMemoryError: Java heap space` 连接彻底断开。
