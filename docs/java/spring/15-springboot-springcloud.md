---
title: Spring Boot 自动装配与微服务组件原理
hide_title: true
sidebar_label: 自动装配与微服务组件
---
...
2. `@ComponentScan`：扫描包，默认扫描当前主类所在的包及其子包。
...
sequenceDiagram
    ...
    Provider->>Nacos: 1. 注册服务实例 (发送 POST 请求)
    Provider->>Nacos: 2. 发送心跳维持健康状态 (每 5 秒一次)
    Consumer->>Nacos: 3. 订阅并拉取服务实例列表
    Nacos-->>Consumer: 4. 返回可用服务实例列表 (缓存在本地)
    Nacos->>Consumer: 5. 实例状态变更时，主动推送最新列表 (UDP)
```

### 2. 核心机制：心跳与健康检查

> **微服务实战之路**：如果您想从零开始搭建完整的微服务体系，请参考 [Spring Cloud 微服务起步：架构演进与全家桶协同实战](./30-springcloud-quickstart.md)。

---

## 三、 Sentinel 流量治理核心原理

我们已经在 [Sentinel 核心原理：高可用流量治理与故障容错之道](./27-sentinel-governance.md) 中详细探讨过 Sentinel。它是保障微服务不因流量洪峰崩溃的“防火墙”。
...
- **临时实例（默认）**：
  - 采用**客户端上报心跳**的机制。客户端每 5 秒向 Nacos Server 发送一次心跳包。
  - 如果 Nacos Server 超过 15 秒未收到心跳，会将实例标记为不健康；超过 30 秒未收到心跳，直接剔除该实例。
- **持久实例**：
  - 采用**服务端主动探测**的机制。Nacos Server 会主动探测服务实例的指定端口。
...
```mermaid
graph LR
    subgraph "Sliding Window (1s)"
        B1["Bucket 1: 0-200ms"] --- B2["Bucket 2: 200-400ms"] --- B3["Bucket 3: 400-600ms"] --- B4["Bucket 4: 600-800ms"] --- B5["Bucket 5: 800-1000ms"]
    end
```
