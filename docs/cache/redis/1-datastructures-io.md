...
2. **实现更简单、易维护**：红黑树在插入和删除时需要进行复杂的旋转 and 变色来维持平衡。而跳表只需通过**随机函数**决定新节点的层数，插入 and 删除只需修改相邻节点的指针，实现非常简单。
   - **补充：跳表的随机层数生成算法**：Redis 的跳表最大层数为 32 层（部分版本 64 层）。每次插入新节点时，会生成一个 `[0, 1)` 的随机数，如果小于 $p$（Redis 中 $p=0.25$），层数就加 1。这意味着层数 $n$ 的概率为 $p^{n-1}(1-p)$，使跳表高度保持在 $O(\log N)$。
...
Redis 的 Hash 类型和全局 Key-Value 映射都是通过 **Dict**（字典）实现的。其底层是一个哈希表，但在扩容或收缩时采用 **渐进式 Rehash**（Incremental Rehash）机制，避免了由于一次性迁移海量数据而导致的线程卡顿。

#### 字典的底层结构

Redis 中的字典包含了两个哈希表 `ht[0]` 和 `ht[1]`（类型为 `dictht`），以便于进行渐进式 Rehash。每个哈希表底层是一个哈希表节点数组（`dictEntry** table`），采用**链地址法（拉链法）**来解决哈希冲突。

- **哈希算法**：Redis 主要使用 **MurmurHash3** 算法来计算键的哈希值，该算法在键长度较短时仍能保持极高的随机分布均匀性与极快的计算速度。
- **冲突解决**：采用单向链表，新节点总是插入到链表的表头（头插法，为了性能考虑）。

#### Rehash 触发条件与容量计算

- **扩容 (Expand)**：
  - 未执行后台持久化（`BGSAVE` / `BGREWRITEAOF`）且装载因子 $\ge 1$。
  - 正在执行后台持久化且装载因子 $\ge 5$（这是为了尽量避免在操作系统执行 **Copy-On-Write** 写时复制时进行内存页拷贝，从而减少物理内存消耗）。
  - 新容量为第一个大于等于 `used * 2` 的 $2^n$ 数值。
- **收缩 (Shrink)**：
  - 当装载因子 $\le 0.1$ 时触发收缩，新容量为第一个大于等于 `used` 的 $2^n$ 数值（最小为 4）。
...
- **小步分摊**：将整个 Rehash 操作拆分成无数个小步骤，每次执行一个 Redis 读写命令，或者通过定时任务，仅迁移 `ht[0]` 某个槽（Sliding Window）的数据到 `ht[1]` 中，把耗时 $O(N)$ 的迁移均摊到了各次请求 $O(1)$ 中。
- **并发请求处理**：
  - **读/删/改操作**：先在 `ht[0]` 寻找，若找不到则到 `ht[1]` 寻址。在 Rehash 期间，这些操作会在两个哈希表上进行。
...
当一个 **Set（集合）** 类型只包含整数，且元素数量不多时，Redis 会使用 **Intset（整数集合）** 作为底层实现。
...
typedef struct intset {
    ...
    int8_t contents[]; // 保存元素的数组，按从小到大顺序紧凑排列，无重复
} intset;
```

#### 升级机制 (Upgrade)

当我们要向整数集合中添加一个新元素，并且该新元素的类型比当前集合所有元素的类型都要长时（例如，当前编码为 `INTSET_ENC_INT16`，新插入的元素是一个 32 位整数），整个整数集合需要先进行**升级**：

1. **按新元素类型扩容**：根据新元素的类型，增大底层数组 `contents` 的空间大小。
2. **元素重排**：将现有元素全部转换成新的类型，并按顺序移动到调整后的对应位置上（从后往前移动，防止覆盖数据）。
3. **写入新元素**：将新元素写入数组中，并将 `encoding` 更新为新的编码方式。

#### 升级的优势与缺点

- **优势**：
  1. **极大地节省内存**：按需分配空间。只有在放入更大整数时才升级，避免了盲目使用 `int64_t` 导致的内存浪费。
  2. **二进制安全**：保证数组中可以安全地存储各种范围的整型数据。
- **缺点**：
  - **不支持降级**：升级操作是不可逆的。一旦升级，即使后续删除了那个导致升级的大整数，数组的编码格式和占用的内存空间也依然会保持在升级后的状态，不会自动降级。

---

### 5. ZipList 压缩列表到 Listpack 紧凑列表的演进
...
- **多线程 I/O 工作流**：
  1. 主线程负责接收建连请求，将对应的 Socket 分配给 I/O 线程。
  2. I/O 线程负责读取 Socket 数据并解析出 Redis 命令。
  3. 主线程**单线程串行**执行所有的命令，确保并发安全。
  4. I/O 线程负责将主线程执行的结果通过 Socket 回写给客户端。
