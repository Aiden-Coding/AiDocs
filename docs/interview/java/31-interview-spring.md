...
详细生命周期代码追踪与流程图，请参参见 [Bean 生命周期](./0-bean-lifecycle.md) 与 [IoC 与 AOP 深度解析](./1-ioc-aop.md#一-spring-bean-的生命周期)。
...
有关这两个注解的源码级分析，请参考 [常用注解底层解析](./5-annotations.md#一-核心装配注解autowired-与-resource)。
...
完整的 12 种失效场景及对应的解决方案，请参见 [事务传播与失效深度解析](./4-transaction.md#二-声明式事务-transactional-失效的-12-种场景)。
...
详细细节可参考 [常用注解底层解析](./5-annotations.md#三-配置类注解configuration-深度工作机制)。
...
详细原理与代码案例，请参考 [事件驱动与业务解耦](./7-spring-events.md)。
...
详细原理与生产实战，请参考 [缓存抽象与原理](./16-spring-cache.md)。
