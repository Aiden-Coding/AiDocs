...
## 56. 深入拆解 Google Percolator 分布式两阶段提交模型的核心组件

### 💡 典型大厂连环追问

* **追问 1**：在没有集中式高可用协调者的前提下，如果负责写锁和决断的 **Primary Key 所在的节点发生了物理崩溃/宕机**，事务该如何回滚或继续提交？另外一个 Secondary Key 所在的并发读写节点是如何不依赖协调者而进行“锁状态反查与自决断”的？
* **追问 2**：在 Percolator 模型下，读请求需要加锁吗？读写并发时，如何利用时间戳（TSO）实现真正的 Mvcc 无锁高并发读取？

### 🛠️ 核心解析答题要点

1. **摒弃传统 2PC 的去中心化演进**：

   传统的 2PC 必须依赖一个强一致、高可用的外部事务协调者（Coordinator），一旦协调者发生了单点物理宕机，整个集群就会锁住并失效。
   Google Percolator 把“协调者的职能”优雅地写入到了**底层列式存储底底座的数据键（Primary Key）自身的锁状态里**。

2. **两阶段锁工作内核机制**：

   - **第一阶段（Prewrite）**：
     - 用户在分布式节点中挑选出一条记录作为 **Primary Key**，其余的作为 **Secondary Key**。
     - 开启事务，向所有物理列节点提交写入，在各行尝试加锁：
       - **Primary 锁**：写入 Primary 锁，它会携带当前的事务时间戳（Start TS）以及相关的事务状态元数据。
       - **Secondary 锁**：写入各个 Secondary 锁，它们的核心特点是：**完全不带有状态，它们内部仅仅存放指向 Primary Lock 的物理引用指针（Pointer）**。
   - **第二阶段（Commit）**：
     - 真正决断事务生死，**只需要且只去提交/释放 Primary Lock**！
     - 只要 Primary 锁成功被清除并在 Write 列写入 Commit TS（提交成功），不管此时 Secondary Key 的锁是否释放，在物理逻辑层面，**该事务已经被宣告彻底提交成功！**
     - 至于 Secondary Locks，后续可以通过异步线程、或者其他并发事务在下次读写时顺手“拔掉”并物理清理掉它们，极大地缩短了二阶段同步处理带来的网络风暴。

3. **硬核防崩溃：Primary 崩溃时的“自治与裁决”**：

   如果提交过程中有节点突然崩溃（比如 Primary 刚写完 Commit，还没来得及清 Secondary 锁就宕机了）：
   - 当另一个事务或读取线程在后续运行，撞上残留的 Secondary 锁时：
     1. 该线程会提取 Secondary 锁内部的指针，自动去**反查 Primary Key 的状态**。
     2. 如果在 Primary Key 这一列查到：已有 Commit TS 记录，说明事务已成功提交。该线程会帮助清理此处悬挂的 Secondary 锁，并放行本次业务逻辑。
     3. 如果在 Primary 列查到：依然是 Prewrite 锁定状态，或者是已被清干净，说明事务已经失败/未提交成功。该线程会代为把该 Primary 锁和自己身上无用的 Secondary 锁链条强制进行物理 Rolled Back 回滚，释放脏数据。

4. **MVCC 与无锁读取**：

   - Percolator 依赖全局中心时钟（TSO）。读事务获取一个 `Read TS`。
   - 读取任意列数据时，不需要对数据写加任何读物理锁。只需判定：如果在 `Read TS` 之前该行存在写未提交的 Lock 记录：
     - 如果是没有提交的 Lock，读请求会短暂退避自旋等待锁消除或执行上述反查。
     - 否则，直接根据 `Read TS` 从 MVCC 多版本链中读取小于等于 `Read TS` 且最新、已 Commit 的历史镜像值。实现极高吞吐的无锁读。
