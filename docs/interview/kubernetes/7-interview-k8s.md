...
## 8. 为什么在万级 K8s 集群中 eBPF/Cilium 相比 kube-proxy 展现出降维打击般的优势？

- **IPVS/Iptables 对大规模扩缩容的致命加锁瓶颈**：
  传统的 `kube-proxy` (IPVS) 虽优化了转发查找速度，但在面临 K8s 内部 Pod 大批量漫游、扩缩容时，每次变更 Endpoint 通信规则，底层的 Netfilter 均需要全系统执行**全局加锁并重新组装刷新**，导致线上大面积抖动丢包。
- **eBPF/Cilium 的 O(1) 替代机制**：
  Cilium 大胆废弃了 Linux 慢速的 `iptables` 链条，而是基于 **eBPF Map 哈希结构（`BPF_MAP_TYPE_HASH`）** 将 K8s Service IP 和 Endpoint 的映射关系固化在内核态。对规则的每一次增改，全过程是**并发读写、无锁、非阻塞的**，确保在大规模网络震荡下网络转发维持在恒定的 $O(1)$。
- **Sockmap 物理绕过协议栈**：
  Cilium 在套接字层注入 **`sockops` (Socket Operations)**，将同宿主机上的跨 Pod 通信的两个 Socket 极速登记在 `BPF_MAP_TYPE_SOCKMAP` 表中。当有 `write` 触发时，在内核态**直接把数据包的内存指针转移挂载到对应 Socket 接收队列的缓冲区**中，**物理免除了 TCP/IP 协议栈和 Veth Pair 多次内存拷贝开销**，直接将通讯耗时削减至单进程级别。
- **零 Sidecar 侵入 Hubble 可观测性**：
  Cilium 借助 eBPF 在内核 Linux 系统调用处的 `kprobe` 与 `Tracepoints` 无侵入钩子，用高性能零内存拷贝的 **Ring Buffer（环形缓冲区）** 向用户态实时、异步上报 HTTP/GRPC 原生报文，直接用零 Sidecar 成本获得了极佳的服务拓扑和 p99 性能大图展现。
