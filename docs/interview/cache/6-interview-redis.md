...
### Q2：高并发下缓存击穿、缓存穿透、缓存雪崩的场景成因以及工业级规避方案
...
- **概念成因**：
  请求查一条**既不在 Redis、也绝不在 MySQL 数据库中**的无端畸形数据（比如恶意黑客发起的 ID = -1 或各种伪造 UUID 的高频查询）。缓存由于没有该 Key 无法拦截，导致每次请求都直接打到 MySQL 建立连接，引发数据库直接宕机。
...
- **工业最强方案：Redisson 分布式锁 + 双重检查锁 (DCL) / 逻辑过期异步构建**：
...
- **概念成因**：
  **在极短时间内，有大量的不同 Key 在同一时间集体大面积到期；或者是代表 Redis 集群整体发生了物理宕机断网。**
...
- **核心特点**：
  ...
  2. **没有回滚机制 (无原子性保证)**：Redis 事务不保证传统数据库 ACID 中的 Atomicity。如果事务中的某一命令在执行时出错，其他命令依然会继续执行，Redis **不会进行回滚 (Rollback)**。
