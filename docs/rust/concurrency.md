...
由于编译器将异步函数内的局部引用的生命周期跨越了所有的 `.await` 挂起点，导致生成的状态机结构体成为了一个**自我引用结构体 (Self-Referential Struct)**。在内存中移动这类结构体会产生毁灭性的悬空指针与未定义行为。

为了确保这类异步任务能够安全运行，Rust 引入了 `Pin` 机制。

> [!TIP]
> 关于 `Future` 惰性轮询原理、自我引用的物理内存移动危机、`Pin` 与 `Unpin` 的详细推导以及如何手写一个带 Waker 的 `TimerFuture` 实例，请参见我们的专栏深水区：
> **[异步底层剖析：Future 与 Pin 机制](async-under-the-hood.md)**。

这里我们重点来看在实际中如何安全地对包含 `Pin` 字段的复杂 Future 进行投影：
