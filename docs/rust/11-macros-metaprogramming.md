...
### 1. 宏的卫生性 (Macro Hygiene)

在大多数宏系统中，宏在展开时往往会在调用它的上下文作用域中引入变量名。如果这些变量名与调用者作用域中的现有变量重名，就会产生意外的覆盖或污染，这种问题被称为**非卫生宏**。

Rust 的**声明宏** (`macro_rules!`) 默认是**卫生（Hygienic）**的：
- **局部变量是卫生的**：在声明宏内部声明的变量，在展开时会被编译器赋予独立的内部标记，绝对不会与调用者作用域中的任何同名变量产生冲突。
- **类型、项与特征路径不是卫生的**：如果在宏里定义了结构体，或者引用了外部的项，它会直接在调用处的上下文中解析。因此，在声明宏中编写外部库路径时，最好使用绝对路径（如 `::std::vec::Vec`）。

```rust
macro_rules! clean_macro {
    () => {
        let x = 42; // 这个 x 是卫生的，不会污染调用处的 x 变量
    };
}
...
fn main() {
    let x = 10;
    clean_macro!();
    println!("{}", x); // ✅ 输出 10，完全不受宏内部变量的影响！
}
```

相反，**过程宏 (Procedural Macros)** 默认是**非卫生**的。过程宏的变量名直接在生成的代码中生效。如果在过程宏中引入了辅助变量，必须通过显式指定 `Span`（如引用 `Span::mixed_site()` 或 `Span::call_site()`）来赋予其不同的卫生性绑定。

### 2. 调试宏代码 (cargo-expand)

由于宏在编译期展开，一旦宏内部代码出错，或者生成的代码不符合预期，排查起来非常棘手。Rust 社区提供了一个极为强大的第三方开发辅助工具：`cargo-expand`。

`cargo-expand` 会自动运行 Rust 编译器前端展开所有的声明宏、过程宏、以及派生特征，并将生成的纯 Rust 源码直接打印在终端上。
...
通过查看展开后的实际代码，你能够像调试普通 Rust 文件一样轻松定位元编程生成的错漏。
