...
在程序中每个引用计数变化的点，会打印出引用计数，其值可以通过调用 `Rc::strong_count` 函数获得。这个函数叫做 `strong_count` 而不是 `count` 是因为 `Rc<T>` 也有 `weak_count`；在 [“避免引用循环：将 `Rc<T>` 变为 `Weak<T>`”](ch15-06-reference-cycles#避免引用循环将-rct-变为-weakt) 部分会讲解 `weak_count` 的用途。
...
[preventing-ref-cycles]: ch15-06-reference-cycles#避免引用循环将-rct-变为-weakt
