...
注意这里必须使用 [“高级 trait”][advanced-traits]<!-- ignore -->  部分讲到的完全限定语法，因为存在多个叫做 `to_string` 的函数；这里使用了定义于 `ToString` trait 的 `to_string` 函数，标准库为所有实现了 `Display` 的类型实现了这个 trait。
...
这段代码正好可以编译。关于 trait 对象的更多内容，请回顾第 17 章的 [“为使用不同类型的值而设计的 trait 对象”][using-trait-objects-that-allow-for-values-of-different-types]<!-- ignore --> 部分。

接下来让我们学习宏！

[advanced-traits]:
ch19-03-advanced-traits.html#高级-trait
