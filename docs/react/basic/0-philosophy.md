...
3. **渲染过程中不能产生任何副作用**（所有副作用必须被严格隔离至 `useEffect` 或事件处理函数中）。

#### 💡 概念透析：什么是“副作用” (Side Effect)？

**副作用**是指组件在计算并输出 UI (JSX) 之外，对**外部世界**产生的任何可观测的改动或影响。在 React 的核心设计中，渲染阶段必须是绝对“纯净”的，这是为了保障并发渲染的中断、重试以及状态更新的可预测性。

##### 1. 常见的副作用类型

- **修改外部变量**：改写在组件渲染函数之外声明的状态（如直接修改全局变量）。
- **操作原生 DOM**：使用 `document.getElementById`、`window.addEventListener` 等手动修改 React 托管之外的 DOM 树或全局对象。
- **异步/时间控制**：设置 `setTimeout`、`setInterval` 定时器。
- **数据 I/O 操作**：发起 HTTP 请求（`fetch`/`axios`）、读写 `localStorage`/`cookie` 等。

##### 2. 为什么不能在渲染过程中产生副作用？

如果渲染函数是非纯的（包含副作用），React 无法预测组件在不同周期的行为。尤其在 **Concurrent Mode (并发模式)** 下，React 可能为了优化交互体验而强行打断当前的渲染计算并抛弃结果，稍后重新渲染。
1. **结果不可预测**：组件无法保障“相同输入产出相同 UI”（破坏 $UI = f(state)$ 映射）。
2. **触发渲染死循环**：若在渲染函数体中直接调用网络请求并触发 `setState`，将导致 “渲染 -> 发送请求 -> setState -> 触发重渲染 -> 再次请求” 的无限死循环。

##### 3. 副作用的正确隔离方式

- **交互引起的副作用**：如点击保存按钮、提交表单等，应当完全隔离在**事件处理函数 (Event Handlers)** 中。
- **挂载/更新自动触发的副作用**：如组件加载时获取数据、开启全局定时器，应当通过 **`useEffect` Hook** 声明，交由 React 在浏览器完成视图渲染并绘制到屏幕后，安全地异步执行。
