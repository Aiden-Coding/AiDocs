...
## 六、 InnoDB Cluster 高可用架构

InnoDB Cluster 是 MySQL 官方基于 MGR（Group Replication）的全栈封装，包含 **MySQL Shell**、**MySQL Router** 和 **InnoDB ReplicaSet** 三大组件，提供自动化部署与故障切换能力。

### 1. 架构组成

```mermaid
graph TD
    subgraph "InnoDB Cluster (3 节点 MGR 集群)"
        Node1[(MySQL 1)] -->|Paxos 共识| Node2[(MySQL 2)]
        Node2 -->|Quorum 2/3| Node3[(MySQL 3)]
        Node3 -->|Group Communication| Node1
    end
    subgraph "代理层"
        Client[客户端应用] -->|自动路由| Router[MySQL Router]
        Router -->|读写分离| Node1
        Router -->|只读流量| Node2
        Router -->|只读流量| Node3
    end
...
- **MySQL Shell**：提供 `dba` 命令一键部署集群（`dba.createCluster()`、`dba.addInstance()`）。
- **MySQL Router**：轻量级代理，自动感知主从切换并路由流量（6446 读写端口、6447 只读端口）。
- **InnoDB ReplicaSet**：Cluster 的逻辑抽象，通过 `cluster.status()` 查看集群状态。
...
**故障切换**：Primary 宕机后，剩余节点通过 Paxos 协议选举新主，Router 10 秒内发现并重定向流量。

### 4. 与纯 MGR 对比

| 特性 | 纯 MGR | InnoDB Cluster |
| :--- | :--- | :--- |
| **部署复杂度** | 高（手动配置 MGR、Binlog、GTID） | 低（MySQL Shell 一键部署） |
| **客户端感知** | 需自行实现连接池重连 | MySQL Router 自动 failover |
| **监控能力** | 依赖第三方 | 内置 `cluster.status()` JSON 输出 |
