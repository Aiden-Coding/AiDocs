...
当数据剧增到单库无法承受、或单表数据破千万（B+树可能恶化为四层带来多一次磁盘读写）时，必须进行数据切分（Sharding）。
...
- **水平分库**：当单台物理机的存储、CPU 和 I/O 天花板被触及，此时我们不但要分表，还要将这些表散布到多台不同的 MySQL 物理服务器或集群节点上（分库分表）。
...
2. **跨库 JOIN 查询困局**：
   ...
   - **应对**：通过在应用服务层编码分别查询各表后再于内存中组装（Scatter-Gather 模式）；或在各分片库中建立“全局表（字典表）/广播表”，将其复制到每一个分片库，允许原本需要在多库中 Join 的查询在单库内完成。
