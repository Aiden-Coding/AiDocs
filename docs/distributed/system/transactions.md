...
- **问题描述**：
  ...
  - 如果外层方法在后续步骤中抛出异常，`@GlobalTransactional` 会触发全局回滚，通过 `undo_log` 还原数据。
  - **致命陷阱**：如果内层本地事务因为某些原因（如数据库连接超时）回滚了，但外层方法捕获了异常并继续执行，TC 无法感知到内层本地事务已经回滚，在全局提交时可能会因为找不到对应的 `undo_log` 或数据不一致而报错。
- **最佳实践**：
  - 尽量避免在同一个方法链中无节制地混用。
  - 确保外层 `@GlobalTransactional` 包裹的方法内部，所有本地事务的异常都能正确向上抛出，让 Seata 协调器（TC）能够精准感知分支事务的状态。

### 3. Seata TC（事务协调器）的高可用部署
