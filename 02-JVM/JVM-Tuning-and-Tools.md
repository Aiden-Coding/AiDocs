...
#### 排查思路：
1. **获取堆转储快照（Heap Dump）**：
   - 建议在 JVM 启动参数中配置以下参数，在发生 OOM 时自动导出堆快照：
     ```bash
...
2. **使用分析工具**：
   ...
   - 查看 **Histogram**（直方图）和 **Dominator Tree**（支配树），找出占用内存最大的对象。
...
| 工具名称 | 主要功能 | 常用命令示例 |
| :--- | :--- | :--- |
| **`jps`** | 查看正在运行的 Java 进程 | `jps -l` |
| **`jstat`** | 监控 JVM 内存、GC 状态 | `jstat -gcutil <PID> 1000 10` (每秒打印一次GC情况，共10次) |
| **`jinfo`** | 查看和修改 JVM 配置参数 | `jinfo -flags <PID>` |
| **`jmap`** | 导出堆快照、查看内存占用 | `jmap -histo <PID> | head -n 20` (查看占用内存前20的类) |
...
#### ① `dashboard`：系统实时面板
输入 `dashboard`，可以一览系统的整体运行状态，包括线程、内存、GC、Runtime 信息。
...
#### 2. 排查过程
1. 使用 `jstat -gcutil <PID> 1000` 观察，发现 **FGC（Full GC）** 计数每分钟增加 5 次，且每次 FGC 后，老年代（O）的内存占用几乎没有下降（保持在 85% 以上）。
2. 使用 Arthas 的 `dashboard` 观察，确认老年代内存已满。
3. 导出堆快照并用 MAT 分析，发现内存中存在大量的 `LocalCache` 对象，其内部持有一个 `ConcurrentHashMap`。
4. 追溯代码，发现开发人员为了提高性能，设计了一个本地缓存，但**未设置最大容量和过期淘汰机制**。随着商品数据的不断加载，缓存无限膨胀，最终塞满老年代，导致频繁 Full GC。

#### 3. 解决办法
1. 紧急情况下，先重启服务释放内存。
2. 修改代码，将本地缓存替换为 **Caffeine** 或 **Guava Cache**，并严格限制最大容量（如 `maximumSize(10000)`）和写入后过期时间。
3. 重新发布后，老年代内存稳定在 30% 左右，Full GC 频率降为每天 1 次以下，接口响应恢复正常。
