...
- **引入元空间的原因**：
  1. **避免 OOM**：永久代有大小限制（通过 `-XX:MaxPermSize` 设置），当加载的类过多时极易抛出 `java.lang.OutOfMemoryError: PermGen space`。而元空间由于使用本地内存，默认情况下只要不超出物理内存上限，就不会 OOM。
  2. **简化 GC**：永久代的垃圾回收效率极低，合并到元空间后，类元数据的生命周期与类加载器一致，释放时机更清晰，简化了垃圾回收逻辑。
...
#### 漏标问题（存活对象被当成垃圾回收）
在并发标记阶段，用户线程和 GC 线程并发运行，可能会出现以下情况：
1. 灰色对象断开了对白色对象的引用。
...
#### 漏标解决方案：
- **写屏障（Write Barrier）**：在引用关系发生改变时，插入一段特殊代码来记录这种变化。
  - **SATB（Snapshot At The Beginning，原始快照）**：G1 采用。当灰色对象断开对白色对象的引用时，将这个旧的引用记录下来。GC 默认这些被断开的白色对象在本次 GC 中是存活的，留到下次 GC 再清理。
...
1. **染色指针（Colored Pointers）**：
   - 传统的垃圾收集器将 GC 信息（如标记位）存在对象头（Mark Word）中。而 ZGC 直接将这些信息存在**对象引用的指针**中。
   - 在 64 位系统下，ZGC 仅使用 42 位来寻址（支持 4TB 内存，JDK 13 扩展到 16TB），并利用高 4 位来存储 GC 标记信息（Marked0, Marked1, Remapped, Finalizable）。
2. **读屏障（Read Barrier）**：
   - 当用户线程从堆中读取对象引用时，会触发读屏障。
   - 读屏障会检查指针的染色位。如果发现对象已经被移动了（正在进行垃圾回收和整理），读屏障会根据转发表（Forwarding Table）自动将指针更新为移动后的新地址。这被称为**自愈（Self-Healing）**。
3. **并发整理（Concurrent Compact）**：
   - 传统的收集器在整理内存碎片时需要 STW。而 ZGC 配合染色指针和读屏障，实现了**与用户线程并发进行的对象移动和整理**，从而消除了整理阶段的停顿。

---

## 四、 高频面试题与追问

### Q1: 为什么 ZGC 的停顿时间能控制在 10ms 以内？
...
### Q2: 什么是 Minor GC、Major GC、Full GC？
- **答**：
  - **Minor GC (Young GC)**：只针对新生代（Eden、Survivor）的垃圾回收。触发非常频繁，速度快。
  - **Major GC (Old GC)**：只针对老年代的垃圾回收。目前只有 CMS 收集器会有单独收集老年代的行为。
  - **Full GC**：收集整个 Java 堆和方法区（元空间）的垃圾回收。会导致长时间的 STW，应尽量避免。

### Q3: 哪些场景会触发 Full GC？
- **答**：
  1. **老年代空间不足**：新生代对象晋升到老年代时，老年代剩余连续空间小于晋升对象的平均大小。
  2. **元空间（Metaspace）空间不足**：动态加载了大量的类，导致元空间达到阈值。
  3. **显式调用 `System.gc()`**：建议通过 `-XX:+DisableExplicitGC` 禁用。
  4. **空间分配担保失败**：Promotion Failure 或 Concurrent Mode Failure（CMS 收集器特有）。
