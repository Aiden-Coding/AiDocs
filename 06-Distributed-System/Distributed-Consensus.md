...
. **Proposer** 选择一个唯一的提议编号 $N$，向半数以上（Quorum）的 **Acceptor** 发送 `Prepare(N)` 请求
...
**Acceptor** 收到 `Accept(N, V)` 请求后，只要它没有承诺过拒绝编号为 $N$ 的提议，就会通过（Accepted）该提议。
...
**发起投票**：Candidate 向集群中其他节点发送 `RequestVote` 请求
...
**广播复制**：Leader 向所有 Follower 广播 `AppendEntries` 请求

**半数确认**：当 Leader 收到半数以上 Follower 的成功响应后，将该 Entry 标记为**已提交（Committed）**，并应用到本地状态机，向客户端返回成功
