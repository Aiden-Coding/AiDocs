...
```mermaid
graph TD
    A[1. 实例化 Instantiate] --> B[2. 属性赋值 Populate]
    B --> C[3. 初始化 Initialize]
    C --> D[4. 销毁 Destroy]
```
...
1. **实例化（Instantiation）**：
   - 通过反射或 CGLIB 动态代理创建 Bean 的实例对象（分配内存空间）。
   - 对应方法：`createBeanInstance()`。
2. **属性赋值（Populate）**：
   - 解析 Bean 的属性并进行依赖注入（如 `@Autowired`、`@Value`）。
   - 对应方法：`populateBean()`。
3. **初始化（Initialization）**：
   - **Aware 接口回调**：如果 Bean 实现了 `BeanNameAware`、`BeanFactoryAware` 等接口，Spring 会将对应的属性注入进去。
   - **BeanPostProcessor 前置处理**：执行所有已注册的 `BeanPostProcessor` 的 `postProcessBeforeInitialization()` 方法（如 `@PostConstruct` 注解就是在此处解析的）。
   - **初始化方法执行**：
     - 如果 Bean 实现了 `InitializingBean` 接口，执行 `afterPropertiesSet()`。
     - 如果配置了自定义的 `init-method`，执行该方法。
   - **BeanPostProcessor 后置处理**：执行所有已注册的 `BeanPostProcessor` 的 `postProcessAfterInitialization()` 方法。**AOP 动态代理通常就是在此阶段生成的。**
4. **销毁（Destruction）**：
   - 当 Bean 不再使用或容器关闭时，触发销毁流程。
   - 如果 Bean 实现了 `DisposableBean` 接口，执行 `destroy()`。
...
sequenceDiagram
    ...
    Spring->>A: 1. 实例化 A (半成品)
    Spring->>Spring: 2. 将 A 的 ObjectFactory 放入三级缓存
    Spring->>A: 3. 属性填充 A，发现依赖 B
    Spring->>B: 4. 实例化 B (半成品)
    Spring->>Spring: 5. 将 B 的 ObjectFactory 放入三级缓存
    Spring->>B: 6. 属性填充 B，发现依赖 A
    B->>Spring: 7. 从缓存获取 A (三级 -> 二级)
    Note over B,Spring: 触发 A 的 ObjectFactory.getObject()<br/>生成 A 的代理对象(若有AOP)并放入二级缓存
    Spring->>B: 8. 完成 B 的属性填充与初始化
    Spring->>Spring: 9. 将 B 放入一级缓存
    Spring->>A: 10. 完成 A 的属性填充与初始化
    Spring->>Spring: 11. 将 A 放入一级缓存
```
...
public class DefaultAopProxyFactory implements AopProxyFactory, Serializable {
    ...
    public AopProxy createAopProxy(AdvisedSupport config) throws AopConfigException {
        // 1. optimize: 是否对 CGLIB 代理进行优化
        // 2. proxyTargetClass: 是否强制使用 CGLIB 代理（如配置了 @EnableAspectJAutoProxy(proxyTargetClass = true)）
        // 3. hasNoUserSuppliedProxyInterfaces: 目标类是否没有实现接口
...
#### 核心实现：`ReflectiveMethodInvocation.proceed()`
```java
